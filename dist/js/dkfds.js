...
__webpack_require__.d(__webpack_exports__, {
  ...
  NewDatePicker: () => (/* reexport */ new_datepicker),
...
;// ./src/js/components/new-date-picker-utils.js
...
function placeDateWithinMinMax(minDate, maxDate, dateToCorrect) {
...
;// ./src/js/components/new-datepicker.js
...
NewDatePicker.prototype.init = function () {
  ...
  this.datePickerWrapper.addEventListener('keydown', e => {
    ...
    switch (key) {
      ...
      case 'Tab':
        ...
        } else if (e.target === firstFocusableElement && e.shiftKey && isDialog(this.datePickerWrapper)) {
          e.preventDefault();
  ...
  this.datePickerWrapper.querySelector('.selected-month').addEventListener('change', e => {
    ...
    const year = parseInt(this.datePickerWrapper.querySelector('.selected-year').value, 10);
    const newDaysInMonth = daysInMonth(numbersToDate(year, month, 1));
    if (newDaysInMonth < day) {
      day = newDaysInMonth;
    }
...
/* harmony default export */ const new_datepicker = (NewDatePicker);
;// ./src/js/dkfds.js
...
var init = function (options) {
  ...
  */
  ...
  for (let d = 0; d < datePickers.length; d++) {
    new new_datepicker(datePickers[d]).init();
  }
